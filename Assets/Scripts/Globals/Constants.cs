using System.Collections;
using System.Collections.Generic;
using UnityEngine;

// Global constants that may be used by multiple classes.
public static class Constants
{
    public const int HEALTH_PER_ENDURANCE = 10;
    public const int HEALTH_PER_STRENGTH = 5;
    public const int QUICK_AP_COST = 2;
    public const int ATTACK_AP_COST = 4;
    public const int MAX_PARTY_SIZE = 6;
    public const string SCENE_TOWN_MENU = "Town";
    public const string SCENE_COMBAT_REWARDS = "CombatRewards";
    public const string SCENE_ARENA_COMBAT = "Combat";
    public const string SCENE_PRE_COMBAT = "PreCombat";
}
